function computerAI(player) {
  ...
  if (detectedShip.length) {
    ...
    // or already shot
    const filteredPossibleShots = possibleShots.filter((shot) => availableShots.includes(shot));
    if (filteredPossibleShots.length) {
      return filteredPossibleShots[Math.floor(Math.random() * filteredPossibleShots.length)];
    }
  }
